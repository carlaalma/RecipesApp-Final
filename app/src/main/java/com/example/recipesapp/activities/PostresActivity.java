...
public class PostresActivity extends AppCompatActivity {
        ...
        if (cursor != null && cursor.moveToFirst()) {
            do {
                try {
                    ...
                    Log.d("PostresActivity", "Receta cargada: " + receta.getTitulo());
                    recetas.add(receta);
                } catch (Exception e) {
                    Log.e("PostresActivity", "Error al procesar receta: " + e.getMessage());
                }
            } while (cursor.moveToNext());
            cursor.close();
        } else {
            Log.w("PostresActivity", "No se encontraron recetas para 'Postres'.");
        }
        db.close();
    }
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (recetas == null) {
            recetas = new ArrayList<>();
            Log.d("PostresActivity", "La lista de recetas estaba null. Se inicializó.");
        }
    ...
    private void showAboutDialog() {
        ...
        builder.setTitle("Acerca de")
                .setMessage("Esta actividad muestra recetas de postres.")
